{{-- Unit List View (Index) --}}
@extends('admin.layouts.master')

@section('title') Dashboard | Units @endsection
...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="bg-white rounded-lg shadow-md border border-gray-200 overflow-hidden">
        ...
        <div class="px-6 py-4 border-b border-gray-200 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h2 class="text-lg font-semibold text-gray-700 uppercase">Unit Information</h2>
            </div>

            <div class="flex flex-col sm:flex-row items-center gap-3">
                {{-- Search Form --}}
                <form method="GET" action="{{ route('units.index') }}" class="flex items-center w-full sm:w-auto">
                    <div class="relative w-full sm:w-64">
                        <input 
                            type="text" 
                            name="search" 
                            placeholder="Enter unit name to search" 
                        ...
                        @if($list->count() > 0)
                            @foreach($list as $key => $item)
                                <tr class="hover:bg-gray-50 transition duration-150">
                                    ...
                                    <td class="px-4 py-3">{{ $item->unit_value }}</td>
                                    <td class="px-4 py-3">
                                        @if($item->status == 1)
