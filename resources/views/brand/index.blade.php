 @extends('admin.layouts.master')
   @section('title') Dashboard | Brand @endsection
   @section('content')
                        ...
                        <div class="header">
                            ...
                            <div class="form-group">
                                 <form method="get" action="{{route('brand.index')}}">
                                     <input type="text" class="form-control" name="search" placeholder="Enter brand name to search" autocomplete="off" required>
                                         ...
                                         @if($list->count()>0)
                                           @foreach($list as $key=>$item)
                                            <tr>
                                                ...
                                                <td>
                                                  @if($item->image!='')
                                                       <img width="50px" height="50px" src="{{asset('brand_logo/'.$item->image)}}">
                                                      @else 
                                                       <img width="50px" height="50px" src="{{asset('category_logo/no_image.png')}}">
                                                    @endif
                                                </td>
                                                <td>@if($item->status==1) Active @else Inactive @endif</td>
                                                <td><a title="Edit customer" href="{{route('brand.edit',$item->id)}}"><i class="material-icons">edit</i></a></td>
                                            </tr>
                                            @endforeach 
                                        @else 
                                        No Data Found
