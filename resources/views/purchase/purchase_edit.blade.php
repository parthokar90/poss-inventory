                                ...
                                <div class="col-md-4">
                                    <h4>Payment Details</h4>
                                     ...
                                     <div class="icon">
                                      ...
                                      <p>Total: {{number_format(($sub_total+$invoice_details->purchase_vat_amount)-$invoice_details->purchase_discount_amount)}} Tk @php $grand_sub=($sub_total+$invoice_details->purchase_vat_amount)-$invoice_details->purchase_discount_amount; @endphp</p>
                                     ...
                                     @foreach($data as $key=>$item)
                                           @php 
                                            ...
                                            @foreach($item->purchaseItem as $details)
                                             <form action="{{route('purchase.update',$details->id)}}" method="POST">
                                                  {{ csrf_field() }}
                                                  {{ method_field('PATCH') }} 
                                                   <tr>
                                                     <td>{{$details->warehouse}}</td>
                                                      ...
                                                      <td><input type="number" name="qty" min="1"></td>
                                                          <td>
                                                             <button type="submit" name="qty_plus" class="btn btn-success btn-sm" style="margin-top: 3px;padding:3px 9px;" value="qty_plus">+</button> 
                                                          @if($details->total_qty>0)
                                                             <button type="submit" name="qty_minus" class="btn btn-danger btn-sm" style="margin-top: 3px;padding:3px 11px;" value="qty_minus">-</button>
                                                          @endif 
                                                      </td>         
                                                 </tr>
